...
function complexity(filePath, builders)
{
	...
	traverseWithParents(ast, function (node) 
	{
		...
		if (node.type === 'FunctionDeclaration') 
		{
			...
			// 6. Halstead
			traverseWithParents(node, function(child){
				if(child.type === "Identifier" && !halsteadSet.has(child.name)){
					halsteadSet.add(child.name);
					builder.Halstead++;
				}
				if((child.type === "BinaryExpression" && !halsteadSet.has(child.operator)) ||
				(child.type === "LogicalExpression" && !halsteadSet.has(child.operator))){
					halsteadSet.add(child.operator);
					builder.Halstead++;
				}
			})
			

			console.log(halsteadSet)
