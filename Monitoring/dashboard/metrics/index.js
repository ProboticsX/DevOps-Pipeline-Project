...
//let checkbox = process.env.CHECKBOX_IP;
  ...
  //read inventory
  let inventoryFile = fs.readFileSync('/root/inventory.json', (err) => {
	if (err) {
		console.error(err);
		return;
	}
...
let django_app = inventoryFile["green"].publicIP;
let react_app = inventoryFile["blue"].publicIP
//let monitor = inventoryFile["monitor"].publicIP;
/// Servers data being monitored.
var servers = 
[
	//{name: "computer", url:`http://${monitor}:8080`,status: "#cccccc", scoreTrend : []},
	//{name: "checkbox",url:`http://${checkbox}/`, status: "#cccccc",  scoreTrend : [0]},
	{name: "django_app",url:`http://${django_app}:8000/`, status: "#cccccc",  scoreTrend : [0]},
	{name: "react_app",url:`http://${react_app}:3000/`, status: "#cccccc",  scoreTrend : [0]},
];
